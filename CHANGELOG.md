...
- **Smart Camera Streaming**:
  - Implemented generic visibility detection for Camera streams.
  - Stream now automatically pauses when the browser tab is hidden (minimized or background tab) or when the Camera Panel is scrolled out of view.
  - Switches to static snapshot (if configured) when paused to close persistent connections and save bandwidth.
  - Maintains stream if window is visible but unfocused (e.g., on a second monitor).

## [1.2.1] - 2026-01-26

### Fixed
- **Installer script**:
  - Fixed issue where the root $HOME variable was inserted into the Nginx config file instead of the actual home directory path of the user running the script.
